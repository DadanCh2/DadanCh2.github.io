...
function hasHitWall() {
  ...
  */
 if (snake.head.column > COLUMNS || snake.head.column < COLUMNS - ROWS){
  return true;
} else if (snake.head.row > ROWS || snake.head.row < ROWS - ROWS){
  return true;
}
else {
  return false;
...
for (let i = 1; i < snake.body.length; i++) {
    let area = snake.body[i];

    if (area.row === snake.head.row && area.column === snake.head.column) {
      return true; 
    }
  }
  return false;
  ...
  while (!spaceIsAvailable) {
    ...
    */
    ...
    for (let i = 0; i < snake.body.length; i++) {
      var place = snake.body[i];

      if (place.row === randomPosition.row && place.column === randomPosition.column) {
